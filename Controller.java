...
public class Controller {
    ...
    public ArrayList<Data> getAllMembers() {
        ArrayList<Data> members = new ArrayList<>();
        try {
            psmt = conn.prepareStatement("SELECT * FROM 회원관리;");
            rs = psmt.executeQuery();
            while (rs.next()) {
                members.add(new Data(rs.getString("id"),
    ...
    public void deleteMember(String id) {
        try {
            psmt = conn.prepareStatement("DELETE FROM 회원관리 WHERE id = ?;");
            psmt.setString(1, id);
            psmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public boolean isSeatReserved(String seatNumber) {
        try {
            psmt = conn.prepareStatement("SELECT COUNT(*) FROM 좌석예약 WHERE 좌석번호 = ?;");
            psmt.setString(1, seatNumber);
            rs = psmt.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0;
    ...
    public void exitSeatByAdmin(String seatNumber) {
        try {
            psmt = conn.prepareStatement("DELETE FROM 좌석예약 WHERE 좌석번호 = ?;");
            psmt.setString(1, seatNumber);
            psmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void updateMember(String id, String newPassword, String newName, String newTel) {
        try {
            psmt = conn.prepareStatement("UPDATE 회원관리 SET password = ?, name = ?, tel = ? WHERE id = ?;");
